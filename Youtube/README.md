...
1.  **Setup**: Make sure you have your `GEMINI_API_KEY` in your environment variables.
2.  **Install Dependencies**: `pip install -r requirements.txt` (and make sure FFmpeg is in your path).
3.  **Run the Factory**:
    - Execute `main.py` to run the full pipeline.
    - Or use `run.bat` if you're on Windows and want a one-click solution.
