...
PACING — NO DEAD AIR (THIS IS THE #1 RETENTION DRIVER, the actual secret of the original viral videos): NEVER write a silent timer or a "..." segment. Silence makes viewers swipe away — it is exactly why recent videos flopped (retention dropped from ~77% to ~50%). The ENTIRE script must play like ONE continuous, rapid-fire CALL-AND-RESPONSE between characters with zero gaps. After EVERY question, a DIFFERENT character (the sidekick, not the one who asked) IMMEDIATELY blurts the obvious trap answer OUT LOUD as a fast tease ("Krabby Patty! Everyone says Krabby Patty..."), then the asker delivers the elimination INSTANTLY. This 3-beat host-asks / sidekick-blurts / host-eliminates rhythm between two voices is the proven pace. Set show_timer=true on that blurt segment so the clock flashes for a beat, but a REAL character ALWAYS speaks it. A segment with speaker="Timer" and text="..." is BANNED — never emit one.
